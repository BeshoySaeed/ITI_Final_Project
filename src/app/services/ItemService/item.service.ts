...
export class ItemService {
  ...
  getItems(): Observable<Item[]> {
    return this.genericService.getAll('item').pipe(
      map((item: any) => {
        return item.data;
      })
    );
  ...
  getItemById(id: number): Observable<any> {
    return this.genericService.getById('item', id).pipe(
      map((item: any) => {
        return item.data;
      })
    );
  ...
  addItem(object: any): Observable<any> {
    return this.genericService.post('item', object).pipe(
      map((item: any) => {
        return item.data;
      })
    );
  ...
  deleteItem(id: number): Observable<Item> {
    return this.genericService.delete('item', id).pipe(
      map((item: any) => {
        return item.data;
      })
    );
  ...
  editItem(id: number, object: any): Observable<Item> {
    return this.genericService.update('item', id, object).pipe(
      map((item: any) => {
        return item.data;
      })
    );
